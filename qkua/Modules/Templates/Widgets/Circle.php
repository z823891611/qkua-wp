...
class Circle {
    ...
    //小工具slug
    protected $widget_slug = 'qk_widget_circle';

    //短代码名
    protected static $shortcode = 'qk_widget_circle';
    ...
    public static function widget( $args, $instance ) {
        ...
        $html = '<ul class="widget-circle-list">';

        // 遍历分类并输出
        if (!empty($circles) && !is_wp_error($circles)) {
            foreach ($circles as $circle) {
                
                $weight = get_term_meta($circle->term_id, 'qk_hot_weight', true);
                
                // 获取分类的文章数量
                $post_count = $circle->count;
                $link = get_term_link($circle);
                
                $img = qk_get_thumb(array('url'=>get_term_meta($circle->term_id,'qk_tax_img',true),'width'=>150,'height'=>150)) ?? '';
                //$cover = qk_get_thumb(array('url'=>get_term_meta($term->term_id,'qk_tax_cover',true),'width'=>1200,'height'=>300)) ?? '';
        
                //echo '<a href="' . get_term_link($circle) . '">' . $circle->name . '</a> - 浏览量：' . $views . ' - 文章数量：' . $post_count . '<br>';
                $html .= '
                        <li class="circle-item">
                            <div class="circle-info-box">
                                <a class="circle-image" href="'.$link.'">
                                    <img src="'.$img.'" width="48" height="48" class="circle-image-face w-h">
                                </a>
       ...
       // 如果 $widget 是空的， 重建缓存
        if ( empty( $widget )) {
            ...
            $widget .= '<div class="circle-widget"> '.$html.'</div>';
            $widget .= '</div>';
