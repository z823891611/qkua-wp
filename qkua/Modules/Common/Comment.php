...
class Comment{
    ...
    public function insert_comment_ip_location($comment_id,$comment){
        ...
        update_comment_meta( $comment_id, 'qk_comment_ip_location',$data);  
    }
    ...
    public static function comment_callback($comment, $args, $depth){
        ...
        //ip属地
        $comment_ip_location = get_comment_meta($comment->comment_ID, 'qk_comment_ip_location', true);
        $ip_location = IpLocation::build_location($comment_ip_location);
        $show = qk_get_option('comment_ip_location_show');
        $ip_location = $ip_location !== '未知' && !empty($show)? '<span>  · '.$ip_location.'</span>': '';
    ?>
        
        <li class="comment-item">
            <figure class="comment-avatar">
                <?php echo $comment_author['avatar'];?>
            </figure>
            <div <?php echo $id_html; ?> class="comment-container">
                <div class="comment-user-info">
                    <?php echo $comment_author['author']; ?>
                </div>
                <div class="comment-content" itemprop="text">
                    <div class="comment-content-text"><?php echo $parent_user;comment_text(); ?></div>
                    <div class="comment-details">
                        <span class="comment-time">
                            <?php echo qk_time_ago($comment->comment_date) ?>
                            <?php echo $ip_location;?>
                        </span>
                        <?php echo self::get_comment_vote($comment->comment_ID) ?>
                        <span class="comment-reply" onclick="qkComments.reply(<?php echo $comment->comment_ID ?>,<?php echo $top_comment_id ?>)">回复</span>
                    </div>
    ...
    public function comment_unapproved_to_approved($comment_id){
        ...
        //保存文章最后一次评论时间，用于查询排序
        update_post_meta($post_id, 'qk_last_comment_date', $comment->comment_date);
        ...
        if($comment->comment_parent > 0){
            $parent_comment_author = get_comment( $comment->comment_parent );
            $parent_comment_author = $parent_comment_author->user_id ? (int)$parent_comment_author->user_id : false;
        }
        ...
        $post_type_name = qk_get_type_name($post_type);
        
        //评论钩子
        do_action('qk_comment_post',array(
    ...
    public static function get_comment_list($data){
        ...
        if(isset($data['orderby']) && $data['orderby']) {
            if (in_array($data['orderby'], array('qk_comment_like'))) {
    ...
    public static function get_comment_author($comment){
        ...
        $user_data = User::get_user_public_data($user_id);//get_userdata($user_id);
        
        $author = "";
        
        
        if($user_data){
			$author .= $user_data['name_html'];//'<a target="_blank" class="user-name" href="'.get_author_posts_url($user_data->ID).'">'.trim(esc_attr($user_data->display_name)).'</a>';
		}else{
    ...
    public static function comment_filters($comment_text) {
        return apply_filters('qk_comment_filters',$comment_text);
    }
    ...
    public static function comment_vote($type,$comment_id){
        ...
        if($user_id == $comment_author) return  array('error'=>'不能点评自己的评论');
        
        //用户喜欢的评论
        $comment_likes = get_user_meta($user_id, 'qk_comment_likes', true);
        $comment_likes = is_array($comment_likes) ? $comment_likes : array();
        $key = array_search($comment_id, $comment_likes); //in_array()
        
        $comment_like = (int)get_comment_meta($comment_id, 'qk_comment_like', true);
        ...
        update_user_meta($user_id,'qk_comment_likes',$comment_likes);
        update_comment_meta($comment_id,'qk_comment_like',$comment_like);
        
        do_action('qk_comment_vote',$comment_id,$user_id,!$key);
    ...
    public static function get_comment_vote($comment_id){
        ...
        return '
            <span class="comment-like dislike '.($is_like  ? 'voted' : '').'" onclick="qkComments.vote(\'like\','.$comment_id.',this)">
    ...
    public static function get_comment_tags($comment){
        ...
        $comment_likes = get_user_meta($post_author, 'qk_comment_likes', true);
        $comment_likes = is_array($comment_likes) ? $comment_likes : array();
        ...
        foreach ($comments as $comment) {
            ...
            $comment_list[] = array(
                ...
                'comment' => array(
                    ...
                    'date' => qk_time_ago($comment->comment_date,true),
                    'post_id' => $comment->comment_post_ID
                ),
                'comment_parent' => $comment_parent ? array(
                    ...
                    'date' => qk_time_ago($comment_parent->comment_date,true),
